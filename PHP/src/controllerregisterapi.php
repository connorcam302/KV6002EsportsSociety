        ...
        if ($this->getRequest()->getRequestMethod() === "POST"){
            ...
            if (!$this->getGateway()->emailUsed($email)){
                if (!is_null($email) && !is_null($password) && !is_null($userign)  && !is_null($userFirst)  && !is_null($userLast)) {
                    if (strpos($email, '@gmail.com') || strpos($email, '@outlook.com') || strpos($email, '@yahoo.com') && strlen($password)>5) {
                        $hashedpassword = password_hash($password, PASSWORD_DEFAULT);
                        $this->getGateway()->registerUser($email,$hashedpassword,$userign,$userFirst,$userLast);
                
                    } else {
                        $this->getResponse()->setMessage("Password or email cannot be used.");
                ...
                $this->getResponse()->setMessage("Sorry! This user already exists!");
